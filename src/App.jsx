...
function AdminPage() {
  ...
  const loadAdminData = async () => {
    try { const [bookData, reservationData] = await Promise.all([adminRequest('/api/admin/books'), adminRequest('/api/admin/reservations')]); setBooks(bookData); setReservations(reservationData) } catch (error) { setMessage(error.message) }
  }
  ...
  const saveBook = async (event) => {
    event.preventDefault()
    try { const saved = await adminRequest(editingId ? `/api/admin/books/${editingId}` : '/api/admin/books', { method: editingId ? 'PUT' : 'POST', body: JSON.stringify(bookForm) }); let finalBook = saved; if (imageFile) { const imageData = new FormData(); imageData.append('image', imageFile); const imageResponse = await fetch(`/api/books/${saved.id}/image`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: imageData }); if (!imageResponse.ok) throw new Error((await imageResponse.json()).error || 'Image upload failed'); finalBook = await imageResponse.json() } setBooks((current) => editingId ? current.map((book) => book.id === editingId ? finalBook : book) : [finalBook, ...current]); setBookForm(emptyBook); setImageFile(null); setEditingId(null); setMessage('Book saved.') } catch (error) { setMessage(error.message) }
  }
