...
function AdminPage() {
  ...
  if (!token) return <div className="admin-login"><div className="admin-login-card"><span className="brand-mark"><BookOpen size={19} /></span><p className="eyebrow">Shelfspace administration</p><h1>Welcome back.</h1><p>Sign in to manage the collection and reservations.</p><form onSubmit={submitLogin} noValidate><label className="field"><span>Email</span><input type="email" value={login.email} onChange={(event) => { setLogin({ ...login, email: event.target.value }); if (loginErrors.email) setLoginErrors({ ...loginErrors, email: '' }) }} aria-invalid={Boolean(loginErrors.email)} />{loginErrors.email && <small className="field-error">{loginErrors.email}</small>}</label><label className="field"><span>Password</span><input type="password" value={login.password} onChange={(event) => { setLogin({ ...login, password: event.target.value }); if (loginErrors.password) setLoginErrors({ ...loginErrors, password: '' }) }} aria-invalid={Boolean(loginErrors.password)} />{loginErrors.password && <small className="field-error">{loginErrors.password}</small>}</label>{loginError && <p className="form-error">{loginError}</p>}<button className="button button-primary" type="submit">Sign in <ArrowRight size={16} /></button></form></div></div>
